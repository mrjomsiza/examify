      ...
      <section className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
        ...
        <div className="space-y-6">
          ...
          <div className="grid gap-4">
            <button className="panel p-5 text-left" type="button" onClick={async () => setStatus(`Covered topic saved: ${(await saveCoveredTopic({ studentId: firstStudent.id, tutorId: profile?.uid, topic: 'Factorisation of trinomials', note: 'Student can expand and factor with moderate support.' })).id}`)}>
            ...
            <button className="panel p-5 text-left" type="button" onClick={async () => setStatus(`Report saved: ${(await saveTutorReport({ studentId: firstStudent.id, tutorId: profile?.uid, note: 'Learner should focus on sign accuracy and checking substitutions.' })).id}`)}>
            ...
            <button className="panel p-5 text-left" type="button" onClick={async () => setStatus(`Question paper saved: ${(await saveQuestionPaper({ subject: 'Mathematics', grade: firstStudent.grade, year: 2026, term: TERMS[1], isNational: false, region: REGIONS[2], notes: 'Algebra-focused drill paper', fileName: 'grade10-june-2026.pdf' })).id}`)}>
